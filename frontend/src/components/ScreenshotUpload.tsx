...
export default function ScreenshotUpload({ onImported }: { onImported?: () => void }) {
  ...
  if (status === 'idle' || status === 'error') {
    return (
      <div>
        {/* 用 label 包裹 input，避免 JS click() 在部分手机浏览器上触发文件管理器 */}
        <label
          className="flex items-center gap-2 px-3 py-2 bg-[#1a1f2e] border border-[#2a3142] text-gray-300 rounded-lg text-sm hover:bg-[#222838] hover:text-white transition cursor-pointer"
        >
          <Scan size={16} />
          <span>截图导入</span>
          <input
