  ...
  return (
    <div className="p-3 md:p-6 h-full flex flex-col">
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        ...
        <div className="ml-auto flex items-center gap-2 md:gap-4 text-[10px] md:text-xs text-gray-500 flex-wrap">
      ...
      <div className="flex-1 min-h-0 flex flex-col md:flex-row gap-3 md:gap-4">
        {/* 博主列表 - desktop sidebar, mobile horizontal scroll */}
        <div className="md:w-52 md:flex-shrink-0 bg-[#0d1220] border border-[#1f2937] rounded-2xl p-3 md:p-4 md:flex md:flex-col">
          ...
          {bloggers.length === 0 ? (
            ...
            <div className="flex md:flex-col gap-2 overflow-x-auto md:overflow-visible md:max-h-full md:overflow-y-auto">
              {[...bloggers].sort((a, b) => b.accuracy - a.accuracy).map((b, i) => (
                <div key={i} className="flex items-center justify-between px-2 md:px-1 py-1.5 rounded-lg hover:bg-[#1f2937] transition whitespace-nowrap md:whitespace-normal shrink-0 md:shrink">
        ...
        {/* 右侧热力图 */}
        <div ref={containerRef} className="flex-1 min-h-[300px] md:min-h-0 bg-[#0d1220] rounded-2xl border border-[#1f2937] relative overflow-hidden">
          {bloggers.length === 0 ? (
